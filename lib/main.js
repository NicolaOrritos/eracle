    ...
    return new Promise( () =>
    {
        ...
        .then( () =>
        {
            ...
            jobs.end( (status) =>
            {
                ...
                {
                    console.err('Finished with error status "%s"', status)
                }

                process.exit(status)
            })
        } )
        .catch( err => console.err('Could not start Eracle. %s', err) && process.exit(1) )
    });
}
...
const options = 'Eracle, the uncomplicated job scheduler \n'
              + ' \n'
              + 'Usage: \n'
              + '  eracle add <JOB_COMMAND> [-p <PRIORITY>] [-o <OUTPUT>] [-w] \n'
              + '  eracle start \n'
              + '  eracle pause \n'
              + '  eracle stop \n'
              + '  eracle status \n'
